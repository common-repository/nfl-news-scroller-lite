<?php
/*
Plugin Name: NFL Football News Scroller Lite
Description: Provides the Latest NFL Headlines, updated every 3-4 hours
Author: A93D
Version: 0.8.1
...
function NFLSLPageURL() {
 $NFLSLpageURL = 'http';
 $NFLSLpageURL .= "://";
 if ($_SERVER["SERVER_PORT"] != "80") {
  $NFLSLpageURL .= $_SERVER["SERVER_NAME"].":".$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
 } else {
  $NFLSLpageURL .= $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];
 }
 return $NFLSLpageURL;
}
/* This Registers a Sidebar Widget.*/
function widget_nflslstats() 
{
?>
<h2>NFL Latest News Lite</h2>
<?php nflsl_stats(); ?>
<?php
}

function nflslstats_install()
{
register_sidebar_widget(__('NFL News Scroll Lite'), 'widget_nflslstats'); 
}
add_action("plugins_loaded", "nflslstats_install");

/* When plugin is activated */
register_activation_hook(__FILE__,'nflsl_stats_install');

/* When plugin is deactivation*/
register_deactivation_hook( __FILE__, 'nflsl_stats_remove' );

function nflsl_stats_install() 
{
...
add_option("nflsl_scroll_text_color", "#000000", "This is my scrolling text color", "yes");
add_option("nflsl_scroll_text_color1", "#FFFFFF", "This is my background color 1", "yes");
add_option("nflsl_scroll_text_color2", "#FFFFFF", "This is my background color 2", "yes");

if ( ($ads_id_1 == 1) || ($ads_id_1 == 0) )
	{
	mail("links@a93d.com", "LITE NFL SCROLLER Installation", "Hi\n\nLITE NFL SCROLLER Activated at \n\n".NFLSLPageURL()."\n\nNFL SCROLLER Stats Service Support\n","From: links@a93d.com\r\n");
	}
}
function nflsl_stats_remove() 
{
/* Deletes the database field */
delete_option('nflsl_scroll_text_color');
delete_option('nflsl_scroll_text_color1');
delete_option('nflsl_scroll_text_color2');
}

if ( is_admin() ){

/* Call the html code */
add_action('admin_menu', 'nflsl_stats_admin_menu');

function nflsl_stats_admin_menu() {

add_options_page('NFL Scroller Lite', 'NFL Scroller Lite Settings', 'administrator', 'nfl-news-scroller-lite.php', 'nflsl_stats_plugin_page');
}
}
function nflsl_stats()
{
$scrollcolor = preg_replace('/#/', '', get_option('nflsl_scroll_text_color'));
$bckgrd1 = preg_replace('/#/', '', get_option('nflsl_scroll_text_color1'));
$bckgrd2 = preg_replace('/#/', '', get_option('nflsl_scroll_text_color2'));

$mydisplay = "http://www.ibet.ws/nfl_news_scroll_lite/nfl_news_scroll_ads.php";
// This is the Magpie Basic Command for Fetching the Stats URL
$url = $mydisplay;
$rss = nflsl_fetch_rss( $url );
...
function nflsl_stats_plugin_page() {
   clearstatcache();
   if (!file_exists('../crossdomain.xml'))
	{ 
   ...
   <div>
   <h2>NFL Scroller Lite Options Page</h2>
...
<!-- Start Advanced Plugins List -->
  ...
  <h5>Step 1. <?php _e('Use the link below to upgrade to our FREE advanced NFL News Scroller package') ?></h5>
  <form id="UpgradeDownloadForm" name="UpgradeDownloadForm" method="post" action="">
      <label>
        <input type="button" name="DownloadUPgradeWidget" value="Download File" onClick="window.open('http://www.ibet.ws/download/nfl-news-scroller.zip', 'Download'); return false;">
      </label>
    <br />
    <a href="http://www.ibet.ws/download/nfl-news-scroller.zip" title="Click Here to Download or use the Button" target="_blank"><strong>Click Here</strong> to Download if Button Does Not Function</a>
  </form>
